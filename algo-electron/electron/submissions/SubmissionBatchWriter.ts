...
export class SubmissionBatchWriter {
  ...
  write(options: SubmissionBatchWriteOptions): SubmissionBatchWriteResult {
    ...
    if (pageProblemId) {
      pageProblemDbId = this.ensureProblem({
        platform,
    ...
    for (const submission of submissions) {
      this.attachProblem(submission, platform, pageProblemDbId)
  ...
  private attachProblem(submission: SubmissionData, platform: string, pageProblemDbId?: string): void {
    if (pageProblemDbId) {
  ...
  private ensureProblem(identity: ProblemIdentity): string | undefined {
