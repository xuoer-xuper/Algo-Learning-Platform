...
`electron/downloads/` 负责普通网页下载的受控落盘策略，以及 `.user.js` 导航进入脚本安装确认页之前的短时请求登记。脚本正文由 `electron/scripts/` 主进程安装器按 installId 读取，不进入普通下载目录、标签会话快照或 renderer。
...
- `userScriptNavigation.ts`：只识别 HTTPS（开发可显式放行 loopback HTTP）的 `.user.js` URL；短时 registry 生成受校验 `installId` 和 `script-install` 内部页路由，并在消费/过期/清空时通知远程安装器取消暂存抓取。
- `index.ts`：模块统一导出入口。
...
- Registry 只保存 URL、净化后的来源文件名和短时 ID，不保存脚本源码、Cookie、header 或凭据；确认/取消/过期后必须消费并联动清理 `UserScriptRemoteInstaller` 的源码、资源与 in-flight 请求。
