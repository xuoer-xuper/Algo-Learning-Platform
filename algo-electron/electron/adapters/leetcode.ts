    ...
    return `(() => {
      ...
      const shouldReportJson = (requestUrl, json) => {
        ...
        if (SUBMISSION_URL_RE.test(url)) {
          ...
          if (id && state.submittedIds[id] && hasSubmissionPayload(json, requestUrl)) return true;
          return false;
        }

        if (/\\/graphql\\/?/.test(url)) {
          const path = typeof location !== 'undefined' ? String(location.pathname || '') : '';
          return /\\/submissions\\/\\d+\\/?/.test(path) && isSubmissionDetailPayload(json) && hasSubmissionPayload(json, requestUrl);
        }
