...
- 暗色模式 token 双值已按 D6 决议预留（`[data-theme="dark"]` 块）。**B5.4 已开闸启用**，并补齐了当初漏掉的三类"依赖底色"值（叠加层/阴影/滚动条滑块）——记录见计划 §11.55。
...
- 桌宠视觉（`pet.css`/`bubble.css`）在 B1 阶段未动——独立风格。B5.5 已落地置顶三模式，并把气泡偏移改为跟随 scale（默认 scale=1 时数值不变），尺寸解耦按实测否掉；记录见计划 §11.52。
...
- ~~暗色模式只预留 token 未启用（D6）。~~ 已在 B5.4 启用，见上文第 1 节。
...
- ~~下一步建议：回到计划主线从 **B0.1（快捷键地基）** 开工~~ —— 已过期。本文是 **B1 阶段的交接快照**，不再跟踪进度；B0-B4、B6 与 B5.1/B5.4/B5.5/B5.6 均已完成，当前实际待办只剩 **B5.2（打磨）与 B5.3（全局动效）**。进度与证据的唯一活台账是 `docs/DESIGN/BROWSER_SHELL_REFACTOR_PLAN.md` §11.3 及各阶段完成记录。
