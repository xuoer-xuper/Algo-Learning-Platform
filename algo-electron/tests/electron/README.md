...
修改启动顺序、窗口创建、IPC 注册时机、preload 路径、`TabManager` 初始化、app 协议、session 权限策略、弹窗接管、userscript frame 时序或 OJ preload 与主进程之间的握手方向时，需要扩展这里。单测里 preload 是 mock 的，任何"main 与 preload 谁先就绪"的时序假设都必须在这里用真实 preload 验证。测试必须使用临时目录和一次性 localhost 服务，不触碰真实登录态；Windows 文件锁清理使用有限重试，不得无限等待。
