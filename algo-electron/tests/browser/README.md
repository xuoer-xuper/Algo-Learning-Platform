...
- `browserLayout.test.ts`：验证主进程布局契约的 36/42/38/78 派生关系，以及 renderer CSS 变量注入。
...
- `tabManagerSession.test.ts`：有序 web 标签的稳定 ID/标题/活动项恢复、只挂载活动 view、持久状态事件边界、精确快照字段、创建中途失败全回滚、新 ID 冲突规避，以及崩溃 view 替换创建失败后的标签保留和重试。
