...
- `useCoachMouseCapture.ts`：按鼠标坐标与 `data-coach-interactive` 区域的实际边界统一控制原生点击穿透；透明空白可穿透，拖拽期间保持捕获，气泡移除或面板变化时重新判断静止指针。
...
- Demo 默认不接 LLM。
- L5 升级需二次确认，不直接给完整答案。
- 桌宠、气泡和聊天面板只声明交互区域，不分别注册穿透开关；原生命中测试引发的 enter/leave 不能替代鼠标坐标事实。
