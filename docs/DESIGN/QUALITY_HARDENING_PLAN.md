...
**当前状态（2026-09-05）**：`fb950cf` 已在 2026-09-03 接入无 LLM 时的本地自动提示、每日手动升级额度和异步题目 ID 解析。随后补充比赛切换及服务/页面生命周期校验，过期异步结果在落库、扣额度和展示前丢弃。以下 §11.1–§11.4 保留为接线前的历史查证，不再表示当前待办；每日额度目前为进程内计数，不能据此承诺重启后仍保持当日用量。当前行为契约见 [AI_COACH_ARCHITECTURE](AI_COACH_ARCHITECTURE.md)。
...
### 11.3 接线前：会话 `problem_id` 恒为 null
