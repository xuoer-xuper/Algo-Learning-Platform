...
完成记录：新增 `019_cookie_records` migration 和 `cookieRecordRepository`，只保存 Cookie 名称、domain、过期时间、安全标记和 `sync_excluded=1`；`value_encrypted` 当前保持为空，完整 Cookie 值仍只由 Electron 持久 session 持有。`CookieVault` 读取站点/domain Cookie 时同步刷新元数据，`tests/db/repositories.test.ts` 覆盖值不落库、同步排除和摘要边界。
...
完成记录：`CookieVault` 新增 `getCookiesForSite()` 供 main 内部读取完整 Cookie，并新增 `getSafeSummaryForSite()`、`getSafeSummaryForDomain()` 输出安全摘要；IPC 新增 `cookies:getSiteSummary`、`cookies:getDomainSummary`，preload 暴露 `getCookieSummaryForSite()`、`getCookieSummaryForDomain()`，renderer 不可读取 Cookie value。已同步 IPC README、Cookie README、类型声明和 IPC contract 测试。
...
完成记录：新增 `tests/ai/traceability.test.ts`，使用临时 SQLite 数据库构造题目、提交、访问和标签数据，验证复习建议、薄弱标签和复习计划可回查本地题目/提交/访问/标签统计依据；同时验证 AI context、Markdown 和建议产物不泄漏 Cookie header、源码路径或 `raw_json`。新增 `npm run test:ai` 并纳入 `npm run test:all`，已同步 AI/test README、交接和 changelog。验证通过 `npm run test:ai`、`npm run test:docs`、`npm run test:core`。
...
完成记录：新增 `020_sync_queue` migration，包含 `entity_type`、`entity_id`、`operation`、`status`、`payload_hash`、`created_at`、`synced_at` 和 `error_message`；`docs/sync-compatibility.md` 明确当前只本地预留，不自动上传，且 Cookie、日志、`raw_json` 和本机路径不进入同步。
...
完成记录：新增 `021_sync_metadata_fields` migration，为历史核心表追加 nullable `deleted_at`，并为早期缺少更新时间的 `activity_events`、`rating_history`、`contest_results`、`ai_context_snapshots` 追加 `updated_at` 且用 `created_at` 回填；已同步 `DATABASE_SCHEMA.md` 和 migration README，不改变 Cookie 不同步原则。
...
完成记录：新增 `electron/backup/backupService.ts`，使用 SQLite backup API 生成时间戳 `.sqlite` 备份；设置页新增 `BackupPanel`，可选择目录执行本地数据库备份。该 `.sqlite` 备份仅用于本机恢复，不作为普通同步格式。
...
完成记录：新增 `electron/backup/learningDataExport.ts`，导出 `schema_version=1` 的学习数据 JSON，覆盖 `problems`、`problem_visits`、`submissions`、`user_daily_stats`、`platform_accounts` 和 `rating_history`；导出 metadata 明确排除 Cookie、`sync_queue`、`raw_json`、日志和本机绝对路径。
...
完成记录：新增导入解析、版本校验、预览和确认导入流程；`registerBackupIpc.ts` 在 main 进程暂存待确认导入数据，preload 暴露 `previewLearningDataImport()` 和 `confirmLearningDataImport(overwriteConflicts)`，设置页支持预览后确认导入或覆盖冲突。
...
完成记录：导入预览按题目 `platform + platform_problem_id`、提交 `platform + platform_submission_id`、每日统计 `local_day`、账号 `platform + handle` 和 rating 历史 `platform + account_id + contest_id` 检测冲突；默认冲突阻止导入，用户选择覆盖冲突才更新。策略写入 `docs/sync-compatibility.md`。
...
完成记录：`sync_queue` 不写 Cookie，学习数据 JSON 导出不包含 `cookie_records` 或 Cookie value；`PROJECT_RULES.md`、`DATABASE_SCHEMA.md`、`electron/backup/README.md`、`docs/sync-compatibility.md` 和 `docs/android-readonly-data-interface.md` 均明确 Cookie 不进入同步、导入导出或安卓只读接口。
...
完成记录：新增 `docs/android-readonly-data-interface.md`，定义未来安卓端只读读取 `schema_version=1` 学习数据 JSON 的字段、只读边界、Cookie 禁区和版本兼容策略；明确当前不实现安卓应用。
...
完成记录：新增 `docs/sync-compatibility.md`，说明 `sync_queue`、同步兼容字段、SQLite 备份、JSON 导入导出、可导出表、禁止同步数据、冲突键和默认导入策略；已加入 `docs/README.md`。
...
完成记录：新增 `tests/db/backupImport.test.ts` 并接入 `npm run test:db`，覆盖 SQLite backup 文件生成、学习数据 JSON round-trip、冲突预览阻止静默覆盖、导出不包含 Cookie、`raw_json` 或本机路径；`npm run test:db` 已通过。
...
- `package.json` 提供 `test:core`、`test:ai`、`test:adapters`、`test:submissions`、`test:db`、`test:electron`、`test:ui`、`test:all` 等入口。
- `tests/README.md`、`algo-electron/README.md`、最终验收清单和项目审计文档同步更新。
- 不改变现有测试断言、数据库 schema、IPC/Preload API、Cookie 策略或提交监测行为。

完成记录：已新增 `tests/run-tests.mjs`，按 suite 编排现有测试；`package.json` 新增 `typecheck` 与 `test:*` scripts，后续补入 `test:ai` 并纳入 `test:all`；测试 README、子项目 README、`docs/final-acceptance-checklist.md` 和 `docs/project-hardening-audit.md` 已同步。
...
- 验证通过 typecheck、lint、architecture guard、security guard、IPC contract、AI 规则与可追溯性、用户脚本 metadata、browser、parser、integration、adapter、submissions、DB repository、docs、packaging、Electron startup smoke 和 renderer screenshot。
...
完成记录：2026-07-04 已运行 `npm run test:all` 并通过；当前覆盖 AI 规则与可追溯性测试，全量自动验证结果已写入项目巩固证据、审计、人工验收模板和交接文档。`git diff --check` 仍仅有既有 LF/CRLF 规范提示。
