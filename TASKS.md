...
当前 Phase 0 文档与架构基线已完成，Phase 1 的浏览器基础已有初步实现。

下一步固定顺序：

1. 执行 `P1-009` 建立持久 session。
2. 执行 `P1-010` 到 `P1-013` 验证四个平台登录状态。
3. 执行 `P1-014` 建立 CookieVault。
4. 执行 `P1-017` 建立站点注册表。
5. 执行 `P1-018` 到 `P1-023` 完成 URL 识别与题目识别事件。
6. 执行 `P1-024` 初始化 SQLite 与学习行为记录。
