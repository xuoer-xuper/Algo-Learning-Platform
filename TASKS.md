...
当前 Phase 0 文档与架构基线已完成，下一步进入 Phase 1 桌面 MVP 基础。

下一步固定顺序：

1. 执行 `P1-001` 初始化 TailwindCSS。
2. 执行 `P1-002` 清理默认模板 UI 和无关资源。
3. 执行 `P1-003` 迁移 `BrowserView` 到 `WebContentsView`。
4. 执行 `P1-004` 抽离 `BrowserHost`。
5. 执行 `P1-009` 建立持久 session。
6. 执行 `P1-014` 建立 CookieVault。
7. 执行 `P1-017` 建立站点注册表。
8. 执行 `P1-024` 初始化 SQLite 与学习行为记录。
...
1. 检查所有文档是否存在并互相一致。
2. 执行 `P1-001` 初始化 TailwindCSS。
3. 执行 `P1-002` 清理模板 UI。
4. 执行 `P1-003` 迁移 `BrowserView` 到 `WebContentsView`。
5. 执行 `P1-004` 抽离 `BrowserHost`。
