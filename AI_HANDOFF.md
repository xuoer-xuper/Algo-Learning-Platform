...
Phase 2 已完成。Phase 3 统计计算层已完成：user_daily_stats 表、每日活跃、刷题趋势、AC 趋势、提交趋势、平台分布、停留时间、学习轨迹、复访次数。剩余 Dashboard UI 和连续活跃天数。
...
1. `P3-009`：计算连续活跃天数。
2. `P3-013`：错题列表。
3. `P3-015`：统计页 Dashboard。
4. `P3-016`：原始事件重算聚合工具。

如果只做一个任务，优先做 `P3-015`（Dashboard）。
