...
2026-07-03 本轮结构收口新增验证：`node node_modules\typescript\bin\tsc --noEmit`、`npm run lint`、`npx --yes tsx tests\ipc\ipcContracts.test.ts`、`npx --yes tsx tests\electron\startupSmoke.test.ts`、`npx --yes tsx tests\integration\problemTitleExtractionWiring.test.ts`、`npx --yes tsx tests\submissions\realtimeTabActivation.test.ts`、`npx --yes tsx tests\ui\rendererScreenshots.test.ts`、`npx --yes tsx tests\scripts\userScriptMetadata.test.ts`、`git diff --check`。`git diff --check` 仅报告既有 LF/CRLF 提示。
