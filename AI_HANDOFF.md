...
当前任务状态以 `TASKS.md` 为准：P8-013 到 P8-061 的结构化拆分、文档覆盖和收尾审计已完成；P8-007 changelog 和 P8-008 electron-builder 打包配置作为后续新增产物的持续标准，保持进行中；P8-009 Windows 安装包发布和 P8-012 v1.0 总验收仍未开始，不应随结构巩固自动标完成。
...
- 2026-07-03 补充：根文档一致性已收口。`README.md` 已同步七站支持范围并移除不存在的 Zustand 技术栈描述；`ARCHITECTURE.md` 已更新为当前 WebContentsView、多标签、adapter、repository、AI 和实时提交监测架构；`DATABASE_SCHEMA.md` 已把早期“实现前确认项”改为当前数据库实现约束；历史调研文档已移除 `file:///` 本机绝对链接并补维护边界。
- 高风险：Nowcoder 和 VJudge 已改为网络结果驱动或强身份关联，避免自测/公开状态行误入库；不要重新引入通用 DOM verdict observer 作为这两站的实时入库来源。

当前已验证命令：`npm run lint`、`node node_modules\typescript\bin\tsc --noEmit`、`node node_modules\esbuild\bin\esbuild tests\browser\ojBridge.test.ts --bundle --platform=node --format=esm --outfile=tmp\browser-ojBridge.test.mjs`、`node tmp\browser-ojBridge.test.mjs`、`npx --yes tsx tests\adapters\codeforcesAdapter.test.ts`、`npx --yes tsx tests\adapters\genericTableSiteAdapters.test.ts`、`npx --yes tsx tests\submissions\submissionPageContextResolver.test.ts`、`npx --yes tsx tests\parsers\siteRules.test.ts`、`npx --yes tsx tests\ipc\ipcContracts.test.ts`、`npx --yes tsx tests\electron\startupSmoke.test.ts`、`npx --yes tsx tests\ui\rendererScreenshots.test.ts`、`node node_modules\esbuild\bin\esbuild tests\db\repositories.test.ts --bundle --platform=node --format=esm --external:better-sqlite3 --external:electron --outfile=tmp\db-repositories.test.mjs`、`$env:ELECTRON_RUN_AS_NODE='1'; node_modules\.bin\electron.cmd tmp\db-repositories.test.mjs`。完整 adapter/submission 测试仍应在提交前再跑一遍。

2026-07-03 本轮结构收口新增验证：`node node_modules\typescript\bin\tsc --noEmit`、`npm run lint`、`npx --yes tsx tests\ipc\ipcContracts.test.ts`、`npx --yes tsx tests\electron\startupSmoke.test.ts`、`npx --yes tsx tests\ui\rendererScreenshots.test.ts`、adapter 目录全量测试、submissions 目录全量测试、`tests\db\repositories.test.ts` Electron ABI bundle 测试、`tests\submissions\submissionBatchWriter.test.ts` bundle 测试、Markdown 相对链接检查、README 覆盖检查、`git diff --check`。`git diff --check` 仅报告既有 LF/CRLF 提示。
...
- 设计与审计：`docs/submission-monitoring-design.md`、`docs/renderer-structure-audit.md`、`docs/project-hardening-audit.md`、`docs/final-acceptance-checklist.md`、`docs/troubleshooting.md`、`docs/database-migration-rollback.md`、`docs/leetcode-realtime-verification.md`。
...
1. 先提交当前结构巩固和文档批次，建议提交信息：`refactor: 拆分提交仓库并补齐项目巩固文档`。
2. 用户按 `docs/final-acceptance-checklist.md` 统一手测七站提交监测、手动同步、统计页、题目页、设置页、用户脚本和打包产物。
3. 若手测发现 UI 或站点回归，按对应模块修复并补最近的自动测试；不要扩大到无关重构。
4. 发布前再处理 P8-009 和 P8-012；只有安装、启动、卸载、升级数据保留和总验收全部通过后才能标完成。
