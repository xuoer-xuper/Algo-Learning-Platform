...
- 数据库时间改为本地系统时间（北京时间）。
- Phase 1 剩余任务：P1-010~013 登录验证、P1-040 回归测试。
...
Phase 1 已完成，下一步进入 Phase 2：题目元数据与提交记录。

1. `P2-001`：实现页面标题抓取接口。
2. `P2-007`：建立 submissions 表。
3. `P2-012`：统一 verdict 标准。
4. `P2-002` 到 `P2-005`：各平台题目元数据抓取。

如果只做一个任务，优先做 `P2-001`（页面标题抓取接口）。
